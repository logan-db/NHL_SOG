...
# MAGIC %run
# MAGIC "/Workspace/Users/logan.rupert@databricks.com/NHL_SOG/nhlPredict/src/utils/ingestionHelper"
...
display(gold_model_data_v2.filter(col("gameId").isNull()).orderBy(desc("gameDate")))
...
grouped_df = pk_norm.groupBy(
    ...
    *[sum(column).alias(f"sum_{column}") for column in per_game_columns + base_columns]
)
...
display(
    final_joined_rank.filter(col("playerTeam") == "VAN").orderBy(
        "gameDate", "playerTeam", "teamGamesPlayedRolling"
    )
)
...
display(
    gold_player_stats.filter(
        (col("shooterName") == shooter_name) & (col("gameId").isNotNull())
    )
    .select(
        "gameDate",
        "playerTeam",
...
display(silver_games_schedule_v2.filter(col("gameDate") == "2024-10-08"))
...
for column in base_columns + columns_to_rank:
    ...
    if column in base_columns + per_game_columns:
        ...
        perc_rank_calc = 1 - percent_rank().over(
            Window.partitionBy("teamGamesPlayedRolling", "season").orderBy(order_col)
        )
...
under_3_games = (
    final_joined_rank.filter(col("season") == 2024)
    .groupBy("playerTeam")
    .count()
    .filter(col("count") <= 3)
    .select("playerTeam")
...
first_3_games = final_joined_rank.filter(
    (col("playerTeam").isin(under_3_games_list)) & (col("gameId").isNull())
)
...
for column in rank_roll_columns:
    first_3_games = final_joined_rank.withColumn(
        column,
        lag(column, 1).over(Window.partitionBy("playerTeam").orderBy(desc("gameDate"))),
    )
...
display(
    gold_game_stats_v2.select(
        *[col for col in gold_game_stats_v2.columns if "opp" in col]
    )
)
...
unique_check = (
    gold_model_data_v2.groupBy("gameId", "playerId", "season")
...
assert (
    unique_check.count() == 0
), f"{unique_check.count()} Duplicate records found in gold_model_data_v2"

# COMMAND ----------

display(
    gold_model_data_v2.filter(
        (col("gameId") == "2023020516") & (col("shooterName") == "Sebastian Aho")
    )
)
...
schedule_next_game = df_with_row_number.filter(col("row_number") == 1).drop(
    "row_number"
)
...
upcoming_final_clean = (
    ...
    .withColumn(
        "season", when(col("gameDate") < "2024-10-01", lit(2023)).otherwise(lit(2024))
    )
    .withColumn(
        "gameDate",
...
unique_check = (
    full_season_schedule.groupBy("gameId", "season", "team")
    .agg(count("*").alias("count"))
    .filter("count > 1")
)
...
assert (
    unique_check.count() == 0
), f"{unique_check.count()} Duplicate records found in full_season_schedule"
...
gold_shots_date = silver_games_schedule_v2.select(
    "team",
    "gameId",
    "season",
    "home_or_away",
...
if str(today_date) >= str(
    ...
    player_index_2023 = (
        bronze_skaters_2023_v2.select("playerId", "season", "team", "name")
...
silver_games_schedule = silver_games_schedule_v2.select(
    "team",
    "gameId",
    "season",
    "home_or_away",
