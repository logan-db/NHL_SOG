...
schema = "struct<field1:string, field2:int>"

# Convert the json_string column to a struct type column
test_json = df.withColumn("json_variant", from_json(col("json_string"), schema))

# Drop the original json_string column
test_json = test_json.drop("json_string")
...
spark.sql("DROP TABLE IF EXISTS lr_nhl_demo.dev.test_json")
spark.sql("CREATE TABLE lr_nhl_demo.dev.test_json (id String, variant_col Variant)")

# COMMAND ----------

# MAGIC %sql
# MAGIC INSERT INTO T (variant_col)
# MAGIC SELECT PARSE_JSON(json_str_col) FROM other_table

# COMMAND ----------

spark.sql("DROP TABLE IF EXISTS lr_nhl_demo.dev.test_json")
test_json.write.format("delta").mode("overwrite").saveAsTable("lr_nhl_demo.dev.test_json")
