...
def download_unzip_and_save_as_table(url, dbfs_table_path, table_name, file_format):
    """
    ...
    if file_format == ".zip":
        temp_path = "/tmp/" + table_name + ".zip"
    elif file_format == ".csv":
        temp_path = dbfs_table_path + "/" + table_name + ".csv"
    ...
    if zipfile.is_zipfile(temp_path):
        with zipfile.ZipFile(temp_path, "r") as zip_ref:
            ...
            zip_ref.extractall(dbfs_table_path)
            print(
                f"The file downloaded from {url} was a ZIP file and successfully copied to {temp_path}."
            )
    else:
        print(
    ...
    print(
        f"Table '{table_name}' successfully saved to DBFS at location: {dbfs_table_path}"
    )

    return dbfs_table_path
