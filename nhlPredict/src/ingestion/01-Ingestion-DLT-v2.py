...
def aggregate_games_data():
    ...
    assert gold_player_stats.filter((col('playerId').isNull()) & (col("playerTeam") != "UTA")).count() == 0, f"PlayerId Null Rows {gold_player_stats.filter(col('playerId').isNull()).count()}"
